...
class Particle:
    def __init__(self, q, m, B, E, v_0, polozaj, vrijeme_gibanja, dt):
        ...
        self.a = [(self.q / self.m) * (self.E(self.t[-1]) + np.cross(self.v[-1], self.B(self.t[-1])))]
        

    def __acc_runge_kutta(self, v, B, E):
        return (self.q / self.m) * (E + np.cross(v, B))

    def runge_kutta(self):
        while self.t[-1] <= self.vrijeme_gibanja:
            k_1_v = self.__acc_runge_kutta(self.v[-1], self.B(self.t[-1]), self.E(self.t[-1])) * self.dt
            k_1_polozaj = self.v[-1] * self.dt

            k_2_v = self.__acc_runge_kutta((self.v[-1] + k_1_v / 2), self.B(self.t[-1]), self.E(self.t[-1])) * self.dt
            k_2_polozaj = (self.v[-1] + k_1_polozaj / 2) * self.dt

            k_3_v = self.__acc_runge_kutta((self.v[-1] + k_2_v / 2), self.B(self.t[-1]), self.E(self.t[-1])) * self.dt
            k_3_polozaj = (self.v[-1] + k_2_polozaj / 2) * self.dt

            k_4_v = self.__acc_runge_kutta((self.v[-1] + k_3_v), self.B(self.t[-1]), self.E(self.t[-1])) * self.dt
